...
## Pins down scripts/buildings/building_repair_service.gd: the tick-counted
...
func _test_pulse_lands_on_tenth_call() -> void:
	...
	for i in 9:
		service.advance_tick(buildings, 12.0, player, config_of)
		_expect(
...
func _test_two_pulses_take_twenty_ticks() -> void:
	...
	for _i in 19:
		service.advance_tick(buildings, 5.0, player, config_of)
...
func _test_restore_caps_at_missing_health_and_stops() -> void:
	...
	_advance(service, buildings, 12.0, player, config_of, BuildingRepairServiceScript.TICKS_PER_REPAIR_PULSE)
...
func _test_credit_charge_and_carry_cross_a_whole_credit() -> void:
	...
	for pulse in 3:
		_advance(service, buildings, 1.0, player, config_of, BuildingRepairServiceScript.TICKS_PER_REPAIR_PULSE)
		_expect(
...
func _test_unaffordable_charge_blocks_the_restore() -> void:
	...
	_advance(service, buildings, 4.0, player, config_of, BuildingRepairServiceScript.TICKS_PER_REPAIR_PULSE)
...
func _test_set_repairing_false_clears_carry() -> void:
	...
	_advance(service, buildings, 1.0, player, config_of, 3 * BuildingRepairServiceScript.TICKS_PER_REPAIR_PULSE)
...
func _advance(
		service, buildings: Array[Node], repair_health: float, player, config_of: Callable, ticks: int
) -> void:
	for _i in ticks:
		service.advance_tick(buildings, repair_health, player, config_of)
