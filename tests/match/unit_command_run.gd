...
func _test_carried_cargo_targeting(token: int, local_player, enemy_player) -> int:
	# Attack is issued through the real command bus, via CommandPump -- see
	# tests/match/support/command_pump.gd and _test_stop_shortcut(). Cargo
	# itself must be registered too, not just attacker: target_entity_id has
	# to resolve back to the cargo Node at execution time, or the attack
	# order falls through to attack-ground at the recorded (zero) position
	# instead -- see SimAttackCommand.target_entity_id's doc comment.
	var pump := CommandPumpScript.new()
	var commands := FakeUnitCommandController.new()
	commands.setup(
		null, null, null, null, null, null, [], pump.bus(), Callable(pump, "next_orderable_tick")
	...
	commands.handle_unhandled_input(_mouse_event(MOUSE_BUTTON_RIGHT))
	_expect(
		attacker.attack_targets.is_empty(),
		"the attack on carried cargo must not happen before its scheduled tick is pumped"
	)
	pump.pump(commands)
...
func _test_attack_orders(token: int, local_player, enemy_player, neutral_player) -> int:
	# Attack is issued through the real command bus, via the same CommandPump
	# every command-issuing case wires in -- see
	# tests/match/support/command_pump.gd and _test_stop_shortcut(). Every
	# entity this case names as an attack target (not only the acting units)
	# must be registered too: target_entity_id has to resolve back to the
	# actual Node at execution time, or the order falls through to
	# attack-ground at the recorded (zero, here) position instead -- see
	# SimAttackCommand.target_entity_id's doc comment.
	var pump := CommandPumpScript.new()
	var commands := FakeUnitCommandController.new()
	commands.setup(
		null, null, null, null, null, null, [], pump.bus(), Callable(pump, "next_orderable_tick")
	...
	ally.entity_id = pump.register(ally, SimEntityRegistryScript.Kind.UNIT)
	enemy.entity_id = pump.register(enemy, SimEntityRegistryScript.Kind.UNIT)
	neutral.entity_id = pump.register(neutral, SimEntityRegistryScript.Kind.UNIT)
	...
	commands.handle_unhandled_input(_mouse_event(MOUSE_BUTTON_RIGHT))
	_expect(
		attacker.attack_targets.is_empty() and unable.attack_targets.is_empty(),
		"the enemy click's attack must not happen before its scheduled tick is pumped"
	)
	pump.pump(commands)
	...
	_expect(
		attacker.attack_targets == [enemy],
		"the allied Ctrl-click's attack must not happen before its scheduled tick is pumped"
	)
	pump.pump(commands)
	_expect(
	...
	commands.handle_unhandled_input(_mouse_event(MOUSE_BUTTON_RIGHT, true, Vector2.ZERO, true))
	_expect(
		attacker.attack_targets == [enemy, ally],
		"the neutral and ground Ctrl-clicks must not happen before their scheduled tick is pumped"
	)
	pump.pump(commands)
	_expect(
	...
	commands.handle_unhandled_input(_mouse_event(MOUSE_BUTTON_RIGHT))
	_expect(
		statuses.back() != "Selected units cannot attack this target",
		"the incompatible click's rejection must not be reported before its scheduled tick is pumped"
	)
	pump.pump(commands)
...
func _test_building_attack_order(
	...
	# Attack is issued through the real command bus, via CommandPump -- see
	# tests/match/support/command_pump.gd and _test_stop_shortcut(). Both
	# turret and enemy must be registered: target_entity_id has to resolve
	# back to the enemy Node at execution time, or the order falls through to
	# attack-ground at the recorded (zero, here) position instead -- see
	# SimAttackCommand.target_entity_id's doc comment.
	var pump := CommandPumpScript.new()
	var commands := FakeUnitCommandController.new()
	...
	turret.entity_id = pump.register(turret, SimEntityRegistryScript.Kind.BUILDING)
	enemy.entity_id = pump.register(enemy, SimEntityRegistryScript.Kind.UNIT)
	var enemy_collider := Node.new()
	enemy.add_child(enemy_collider)
	...
	_expect(
		turret.attack_targets.is_empty(),
		"the turret's attack must not happen before its scheduled tick is pumped"
	)
	pump.pump(commands)
	_expect(
